...
/*Insertar*/    
    ...
    router.post('/createPedido', (req, res) => {
      // Recibe los datos del nuevo registro desde el cuerpo de la solicitud (req.body)
      const { Fecha_Pedido, Direccion, Estado_Pedido} = req.body;
  
      // Verifica si se proporcionaron los datos necesarios
      if (!Fecha_Pedido || !Direccion || !Estado_Pedido) {
        return res.status(400).json({ error: 'Todos los campos son obligatorios' });
      }
  
      // Realiza la consulta SQL para insertar un nuevo registro con ID específico
      const sql = `INSERT INTO Pedido (Fecha_Pedido, Direccion, Estado_Pedido) VALUES (?, ?, ?)`;
      const values = [Fecha_Pedido, Direccion, Estado_Pedido];
    ...
    router.post('/createProducto', (req, res) => {
      ...
      // Realiza la consulta SQL para insertar un nuevo registro con ID específico
      const sql = `INSERT INTO Producto (Nombre_Producto, Descripcion, Precio) VALUES (?, ?, ?)`;
      const values = [Nombre_Producto, Descripcion, Precio];
    ...
    router.post('/createDetalle', (req, res) => {
      ...
      // Realiza la consulta SQL para insertar un nuevo registro con ID específico
      const sql = `INSERT INTO Detalle (Cantidad) VALUES (?)`;
  ...
  /*Actualizar*/
    ...
    router.put('/updatePedido/:id', (req, res) => {
      ...
      const sql = `
        UPDATE Pedido
        SET Fecha_Pedido = ?, Direccion = ?, Estado_Pedido = ?
        WHERE Id_Pedido = ?
      `;
  
      const values = [Fecha_Pedido, Direccion, Estado_Pedido, id];
    ...
    router.put('/updateProducto/:id', (req, res) => {
      ...
      const values = [Nombre_Producto, Descripcion, Precio, id];
